...
const Movies = () => {
  ...
  const fetchMovies = async (pageNum = 1) => {
    try {
      const cacheData = await AsyncStorage.getItem(`${tvCache}_page${pageNum}`);
      if (cacheData) {
      ...
      const res = await axios.get(
        ...
        `https://api.themoviedb.org/3/discover/tv?page=${pageNum}`,
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-900">
      <View className="w-full h-[60px]  justify-center p-5">
        <Text className="text-slate-300 text-[20px] font-bold">Tv Shows</Text>
      </View>
